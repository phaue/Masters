...
namespace Detectors_ {
  ...
  class Detector_frib {
      ...
      : id(_id), name(_name), type(_type), calibration(_calibration), setup(_setup) {
        switch (type) {
          ...
          case Pad:
            ...
            frontside_ContactThickness = AUSA::getFrontContactThickness(*setup->getSingleSided(this->name));
   ...
   double getFrontContactThickness() const {return frontside_ContactThickness;};
   ...
   void setFrontContactThickness(double frontcontact) {this->frontside_ContactThickness = frontcontact;}
   //set banana cut
  ...
  private:
    ...
    double frontside_DeadLayer{}, Detector_thickness{}, backside_DeadLayer{}, backside_ContactThickness{}, frontside_ContactThickness{}; // = 0.0 by default
    double BetaCut{};
